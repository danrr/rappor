...
class Params(object):
  ...
  """
    ...
    The names are compatible with the apps/api server.
    """
        return json.dumps({
...
def get_bloom_bits(word, cohort, num_hashes, num_bloombits):
  ...
  """
    ...
    if num_hashes > len(digest):
        raise RuntimeError(f"Can't have more than {len(digest)} hashes")
...
def get_prr_masks(secret, word, prob_f, num_bits):
    ...
    if num_bits > len(digest_bytes):
        raise RuntimeError(f'{num_bits} bits is more than the max of {len(digest_bytes)}')
